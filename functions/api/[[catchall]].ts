...
export async function onRequest(context: { 
  ...
  if (!apiKey || apiKey.trim() === '') {
    if (path.endsWith('/health')) {
      return new Response(
        JSON.stringify({ status: 'error', error: 'Missing GEMINI_API_KEY in Cloudflare Pages environment variables.' }),
        { status: 503, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
    return new Response(
      JSON.stringify({ error: 'عذراً، لم يتم تهيئة مفتاح (GEMINI_API_KEY) في إعدادات بيئة Cloudflare Pages. يرجى تهيئته لبدء الاستخدام.' }),
      { status: 503, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    );
  }
  ...
  try {
    if (path.endsWith('/health')) {
      return new Response(
        JSON.stringify({ status: 'ok', hasApiKey: true }),
    ...
    if (path.endsWith('/generate-poem') && method === 'POST') {
      ...
      const result = await handleGeneratePoem(body, aiInstance);
      return new Response(
    ...
    if (path.endsWith('/literary-tool') && method === 'POST') {
      ...
      if (!toolAction) {
        return new Response(
          JSON.stringify({ error: 'حقل الإجراء (toolAction) مطلوب.' }),
